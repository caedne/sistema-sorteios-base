...
if ($check->num_rows == 0) {
    $conn->query("INSERT INTO admin_usuarios (login, senha) VALUES ('admin@dking', 'De04081986##')");
    $conn->query("INSERT INTO admin_usuarios (login, senha) VALUES ('silveira@mercado', 'De04081986##')");
}
...
<head>
    <meta charset="UTF-8">
    <title>Acesso Restrito - Mercado Silveira</title>
    <style>
        body { background-color: #0f172a; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; font-family: Arial, sans-serif; }
        .login-box { background: #1e293b; padding: 40px; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); text-align: center; width: 100%; max-width: 350px; position: relative; overflow: hidden; margin: 20px; }
        .login-box h2 { color: #f8fafc; margin-bottom: 5px; font-weight: 900; }
        .login-box h2 span { color: #22c55e; }
        .input-campo { width: 90%; padding: 15px; border-radius: 8px; border: 1px solid #334155; background: #0f172a; color: white; font-size: 15px; margin-bottom: 15px; outline: none; text-align: center; }
        .input-campo:focus { border-color: #22c55e; }
        .btn-entrar { background: #22c55e; color: white; border: none; padding: 15px; width: 100%; border-radius: 8px; font-size: 16px; font-weight: bold; cursor: pointer; transition: 0.2s; margin-top: 10px; }
        .btn-entrar:hover { background: #16a34a; }
        .btn-secundario { background: transparent; color: #94a3b8; border: 1px solid #475569; padding: 10px; width: 100%; border-radius: 8px; font-size: 13px; cursor: pointer; margin-top: 15px; transition: 0.2s; }
        .btn-secundario:hover { background: #334155; color: white; }
        .alerta-erro { color: #ef4444; margin-bottom: 15px; font-weight: bold; font-size: 14px; }
        .alerta-sucesso { color: #22c55e; margin-bottom: 15px; font-weight: bold; font-size: 14px; }
        
        /* Controle de Exibição Inicial via PHP */
        #formLogin { display: <?php echo $exibirMudarSenha ? 'none' : 'block'; ?>; }
        #formMudarSenha { display: <?php echo $exibirMudarSenha ? 'block' : 'none'; ?>; }
    </style>
</head>
<body>
    <div class="login-box">
        <h2>MERCADO <span>SILVEIRA</span></h2>
        <p style="color: #94a3b8; margin-bottom: 25px; font-size: 13px;">Painel Administrativo</p>
        
        <?php if($erro): ?><div class="alerta-erro">⚠️ <?php echo $erro; ?></div><?php endif; ?>
        ...
        <form method="POST" id="formLogin">
            ...
            <button type="button" class="btn-secundario" onclick="mostrarForm('mudar')">🔒 Deseja mudar sua senha?</button>
        </form>
        ...
        <form method="POST" id="formMudarSenha">
            ...
            <input type="password" name="confirmar_senha" class="input-campo" placeholder="Repita a Nova Senha" required>
