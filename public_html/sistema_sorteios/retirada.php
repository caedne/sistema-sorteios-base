...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // NOVO: Baixar toda a pesquisa
    if (isset($_POST['baixar_busca'])) {
        $b = $conn->real_escape_string($_POST['baixar_busca']);
        $aba = $conn->real_escape_string($_POST['tab']);
        if (!empty($b)) {
            // Usa a mesma regra da pesquisa: começa com a palavra ou tem espaço antes da palavra (evita 'beto' ao buscar 'eto')
            $conn->query("UPDATE ganhadores_premios gp 
                          ...
                          WHERE gp.status_retirada = 'pendente' AND s.categoria = '$aba' 
    ...
    if (isset($_POST['add_lista'])) {
        $id = (int) $_POST['add_lista'];
        $conn->query("UPDATE ganhadores_premios SET no_carrinho = 1 WHERE id = $id");
    }
    if (isset($_POST['remover_lista'])) {
        $id = (int) $_POST['remover_lista'];
        $conn->query("UPDATE ganhadores_premios SET no_carrinho = 0 WHERE id = $id");
    }
...
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'data'; // NOVO: Filtro de ordenação

// Regra Inteligente: Se pesquisa por nome, evita correspondência no meio de outras palavras
if (!empty($busca)) {
    $where = " AND s.categoria = '$abaAtiva' AND (gp.nome_cliente LIKE '$busca%' OR gp.nome_cliente LIKE '% $busca%')";
} else {
    $where = " AND s.categoria = '$abaAtiva' AND DATE(gp.data_ganho) = '$filtroData'";
}

// Ordenação dinâmica (Data ou Ordem Alfabética por Nome)
if ($sort === 'nome') {
...
// Conta o total
$sql_count = "SELECT COUNT(*) as total FROM ganhadores_premios gp JOIN sorteios s ON gp.sorteio_id = s.id WHERE gp.status_retirada = 'pendente' $where";
...
// Busca com LIMIT e ORDER BY Dinâmico
$sql_lista = "SELECT gp.*, s.titulo FROM ganhadores_premios gp JOIN sorteios s ON gp.sorteio_id = s.id WHERE gp.status_retirada = 'pendente' $where $orderBy LIMIT $limite_por_pagina OFFSET $offset";
$res = $conn->query($sql_lista);

// Cálculo dos itens exibidos
$itens_inicio = ($total_registros > 0) ? $offset + 1 : 0;
$itens_fim = min($offset + $limite_por_pagina, $total_registros);

// ==========================================
// 3. AGRUPAMENTO DO CARRINHO
// ==========================================
...
<head>
    ...
    <style>
        ...
        /* CSS PARA IMPRESSÃO DA TABELA DE PESQUISA */
        @media print {
            ...
            .btn-lista-add, .btn-remover-lista, .btn-retirada-direta, .paginacao, .info-paginacao, .form-busca-retirada { display: none !important; }
            table th:last-child, table td:last-child { display: none !important; } /* Esconde a coluna Ações */
    ...
    <div class="conteudo-principal">
        ...
        <div id="listaSeparacao" class="painel-lista-ativa <?php echo ($total_itens_carrinho == 0) ? 'vazia' : ''; ?>">
            <div class="header-lista-mobile">
                ...
                <form method="POST" onsubmit="return confirm('Deseja CANCELAR toda a lista?')">
            ...
            <div class="corpo-lista-mobile">
                <?php if ($total_itens_carrinho > 0):
                    ...
                    <?php endforeach; else: ?>
                    <p style="text-align:center; color:#94a3b8; padding:30px;">Nenhum item na lista para retirar.</p>
                <?php endif; ?>
            </div>

            <?php if ($total_itens_carrinho > 0): ?>
                <div class="footer-lista-mobile">
                    <form method="POST" onsubmit="return confirm('Confirmar que TODOS os produtos da lista foram entregues?')">
                        <button type="submit" name="finalizar_tudo" class="btn-finalizar-tudo">CONCLUIR TODAS AS ENTREGAS</button>
                    </form>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="painel-tabela-full">
            <table class="tabela-retirada-full">
                <thead>
                    <tr>
                        <th style="width: 30%">DATA / SORTEIO</th>
                        <th style="width: 25%">
                ...
                <tbody>
                            ...
                            <tr style="<?php echo ($v['no_carrinho'] == 1) ? 'background:#f0f9ff; border-left:4px solid #6366f1;' : ''; ?>">
                                ...
                                <td style="display:flex; gap:8px;">
                                    <form method="POST">
                                        ...
                                        <?php if ($v['no_carrinho'] == 0): ?>
                                            <button type="submit" name="add_lista" value="<?php echo $v['id']; ?>" class="btn-lista-add">+ LISTA</button>
                                        <?php else: ?>
                                            <button type="submit" name="remover_lista" value="<?php echo $v['id']; ?>" class="btn-remover-lista">❌</button>
                                        <?php endif; ?>
                                    </form>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="4" style="text-align:center; padding:30px; color:#94a3b8;">Nenhum prêmio pendente encontrado.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
